...
class TerminalTab:
    def __init__(self, notebook, name, on_close_callback, status_bar):
        ...
        self.home_dir = f"/home/{self.real_user}" if self.real_user != 'root' else os.path.expanduser("~")
        ...
        self.notebook.add(self.frame, text=f"💻 {name}")
        ...
        self.dir_label = tk.Label(
            self.top_bar, text=f"📁 {self.current_dir}", bg='#1A1A1A', fg='#44AAFF', font=('Consolas', 9)
        )
        self.dir_label.pack(side=tk.LEFT, padx=10)
        
        self.stop_button = tk.Button(
            self.top_bar, text="⏹️ STOP", command=self.stop_current_process,
            bg='#5A2A2A', fg='#FF4444', font=('Arial', 9, 'bold'), relief=tk.FLAT,
            padx=15, pady=3, state=tk.DISABLED, cursor="hand2"
        )
        self.stop_button.pack(side=tk.RIGHT, padx=10)
        
        self.clear_button = tk.Button(
            self.top_bar, text="🗑️ CLEAR", command=self.clear_screen,
            bg='#2A2A2A', fg='#AAAAAA', font=('Arial', 9), relief=tk.FLAT, padx=15, pady=3, cursor="hand2"
        )
        ...
        self.prompt_label = tk.Label(
            input_frame, text=self.get_prompt_text(), bg='#0C0C0C', fg='#FF4444',
            font=('Consolas', 10, 'bold')
        )
        self.prompt_label.pack(side=tk.LEFT, padx=(10, 5))
        
        self.input_entry = tk.Entry(
            input_frame, bg='#1A1A1A', fg='#00FF00', font=('Consolas', 10),
            relief=tk.FLAT, insertbackground='#00FF00', highlightthickness=0, bd=0
        )
        ...
        self.add_output(f"\n{'='*70}\n", 'info')
        self.add_output(f"🔥 CyberTerm - Вкладка: {name}\n", 'success')
        self.add_output(f"📂 Текущая директория: {self.current_dir}\n", 'info')
        self.add_output(f"👤 Пользователь: {self.real_user}\n", 'info')
        self.add_output(f"{'='*70}\n", 'info')
        self.add_output("✅ Терминал готов. Введите 'help' для списка команд.\n")
        self.add_output("💡 Закрыть вкладку: ПКМ по вкладке → Close\n\n")
    ...
    def stop_current_process(self):
        if self.current_process is not None:
            try:
                ...
                self.add_output("✅ Сигнал остановки отправлен\n", 'success')
                self.process_running = False
    ...
    def show_help(self):
        help_text = """
╔══════════════════════════════════════════════════════════════════════════╗
║  Команды: help, clear, pwd, ls, cd, exit                                ║
╠══════════════════════════════════════════════════════════════════════════╣
║  • При [ПРОЦЕСС] - введите 'y' или 'n' для ответа                      ║
║  • STOP - остановить процесс                                            ║
║  • ПКМ по вкладке -> Close - закрыть вкладку                           ║
...
"""
    ...
    def change_directory(self, path):
        ...
        try:
            ...
            self.add_output(f"✅ Перешли в: {self.current_dir}\n", 'success')
        except FileNotFoundError:
            self.add_output(f"❌ Директория не найдена: {path}\n", 'error')
        except PermissionError:
            self.add_output(f"❌ Нет прав доступа: {path}\n", 'error')
        except Exception as e:
            self.add_output(f"❌ Ошибка: {e}\n", 'error')
...
class ToolTab:
    def __init__(self, notebook, tool_name, on_close_callback):
        ...
        self.frame = ttk.Frame(notebook)
        self.notebook.add(self.frame, text=f"🔧 {tool_name}")
        ...
        self.tool_configs = {
            ...
            "Hydra": {
                "icon": "🐉", "description": "Брутфорс паролей",
                "params": [
                    {"name": "Цель", "key": "target", "default": "127.0.0.1"},
        ...
        tk.Label(title_frame, text=f"{config.get('icon', '🔧')} {tool_name}", bg='#1A1A1A', fg='#00FF00', font=('Arial', 16, 'bold')).pack()
        tk.Label(title_frame, text=config.get('description', 'Инструмент'), bg='#1A1A1A', fg='#666666', font=('Arial', 10)).pack(pady=(5, 0))
        ...
        for i, param in enumerate(config.get('params', [])):
            ...
            tk.Label(row_frame, text=f"{param['name']}:", bg='#1A1A1A', fg='#44AAFF', font=('Consolas', 10, 'bold'), width=12, anchor='e').pack(side=tk.LEFT, padx=(0, 10))
            
            entry = tk.Entry(row_frame, bg='#2A2A2A', fg='#00FF00', font=('Consolas', 10), width=50, relief=tk.FLAT, insertbackground='#00FF00')
        ...
        self.run_button = tk.Button(button_frame, text="▶ ВЫПОЛНИТЬ", command=self.run_tool, bg='#2A5A2A', fg='#44FF44', font=('Arial', 11, 'bold'), relief=tk.FLAT, padx=30, pady=8, cursor="hand2")
        self.run_button.pack(side=tk.LEFT, padx=5)
        
        self.clear_button = tk.Button(button_frame, text="🗑️ ОЧИСТИТЬ", command=self.clear_output, bg='#3A3A3A', fg='#AAAAAA', font=('Arial', 11), relief=tk.FLAT, padx=25, pady=8, cursor="hand2")
        self.clear_button.pack(side=tk.LEFT, padx=5)
        
        output_frame = tk.Frame(self.frame, bg='#0C0C0C')
        output_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=(0, 10))
        
        tk.Label(output_frame, text="РЕЗУЛЬТАТ:", bg='#0C0C0C', fg='#666666', font=('Arial', 9, 'bold'), anchor='w').pack(fill=tk.X, pady=(0, 5))
        ...
        self.output_text = tk.Text(text_container, bg='#0C0C0C', fg='#00FF00', font=('Consolas', 10), wrap=tk.WORD, relief=tk.FLAT, padx=10, pady=10, yscrollcommand=v_scroll.set)
        self.output_text.pack(fill=tk.BOTH, expand=True)
        ...
        self.add_output(f"✅ {tool_name} готов\n", 'success')
    ...
    def run_tool(self):
        ...
        if not config:
            self.add_output("❌ Ошибка конфигурации\n", 'error')
            return
        
        cmd = config['build_cmd'](params)
        if not cmd:
            self.add_output("❌ Не удалось сформировать команду\n", 'error')
            return
        ...
        def target():
            try:
                ...
                self.add_output(f"\n✅ Завершено (код: {result.returncode})\n", 'success')
            except subprocess.TimeoutExpired:
                self.add_output("❌ Таймаут 120 сек\n", 'error')
            except Exception as e:
                self.add_output(f"❌ Ошибка: {e}\n", 'error')
...
class CyberTerminalApp:
    def __init__(self, root):
        self.root = root
        self.root.title("CyberTerm - Профессиональный терминал")
        ...
        tk.Label(logo_frame, text=" CyberTerm", bg='#0C0C0C', fg='#00FF00', font=('Arial', 14, 'bold')).pack(side=tk.LEFT, padx=(5, 0))
        tk.Label(logo_frame, text=" | Professional Terminal", bg='#0C0C0C', fg='#666666', font=('Arial', 10)).pack(side=tk.LEFT)
        
        button_frame = tk.Frame(top_bar, bg='#0C0C0C')
        button_frame.pack(side=tk.RIGHT)
        
        self.new_terminal_button = tk.Button(button_frame, text="+ Новый терминал", command=self.add_terminal_tab, bg='#1A1A1A', fg='#00FF00', font=('Arial', 10), relief=tk.FLAT, padx=15, pady=5, cursor="hand2")
        self.new_terminal_button.pack(side=tk.LEFT, padx=5)
        
        self.new_tool_button = tk.Button(button_frame, text="+ Инструмент", command=self.add_tool_tab_dialog, bg='#1A1A1A', fg='#44AAFF', font=('Arial', 10), relief=tk.FLAT, padx=15, pady=5, cursor="hand2")
        self.new_tool_button.pack(side=tk.LEFT, padx=5)
        
        self.status_bar = tk.Label(root, text=f"✅ Готов | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", bg='#1A1A1A', fg='#666666', anchor='w', padx=10, font=('Arial', 9))
    ...
    def on_tab_right_click(self, event):
        try:
            ...
            if tab_index >= 0:
                ...
                menu.add_command(label=f"Закрыть {tab_text}", command=lambda: self.close_tab(tab_index))
    ...
    def update_status_time(self):
        self.status_bar.config(text=f"✅ Готов | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def add_terminal_tab(self, name=None):
        ...
        self.status_bar.config(text=f"✅ Создан терминал: {name}")
        return tab
    
    def add_tool_tab(self, tool_name):
        tab = ToolTab(self.notebook, tool_name, self.on_tool_closed)
        self.tool_tabs.append(tab)
        self.status_bar.config(text=f"✅ Добавлен инструмент: {tool_name}")
        return tab
    
    def add_tool_tab_dialog(self):
        dialog = tk.Toplevel(self.root)
        dialog.title("Добавить инструмент")
        dialog.geometry("400x350")
        dialog.configure(bg='#1A1A1A')
        dialog.transient(self.root)
        dialog.grab_set()
        
        tk.Label(dialog, text="Выберите инструмент:", bg='#1A1A1A', fg='#00FF00', font=('Arial', 14, 'bold')).pack(pady=20)
        
        tools_frame = tk.Frame(dialog, bg='#1A1A1A')
        tools_frame.pack(fill=tk.BOTH, expand=True, padx=20)
        
        for tool in ["Nmap", "SQLMap", "Netcat", "Hydra", "Metasploit"]:
            btn = tk.Button(tools_frame, text=f"🔧 {tool}", command=lambda t=tool: self._add_tool_from_dialog(dialog, t), bg='#2A2A2A', fg='#00FF00', font=('Arial', 11), relief=tk.FLAT, pady=8, cursor="hand2")
            btn.pack(fill=tk.X, pady=5)
        
        tk.Button(tools_frame, text="Отмена", command=dialog.destroy, bg='#3A3A3A', fg='#AAAAAA', font=('Arial', 10), relief=tk.FLAT, pady=5, cursor="hand2").pack(pady=10)
    ...
    def on_closing(self):
        ...
        if messagebox.askokcancel("Выход", "Закрыть CyberTerm?"):
            self.root.quit()
            self.root.destroy()
